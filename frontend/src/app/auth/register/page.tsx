...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { token, id, name: userName, role: updatedRole } = authResponse.data;
      
      login({ id, name: userName, email, role: updatedRole }, token);
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#eef2ff] p-4 font-sans">
      <div className="w-full max-w-lg p-8 sm:p-10 bg-white rounded-2xl shadow-xl border border-[#e5e7eb] animate-in slide-in-from-bottom-4 duration-500 fade-in">
        <div className="text-center mb-10">
          ...
          <p className="text-sm text-[#6b7280] font-medium mt-2">Set up your workspace to start managing requests.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex flex-col gap-1.5 w-full">
            <label className="text-sm font-semibold text-[#111827]">Account Authority Role</label>
            <select
              value={role}
              onChange={(e) => setRole(e.target.value)}
              className="w-full px-4 py-3 rounded-lg border border-[#e5e7eb] focus:outline-none focus:ring-2 focus:ring-[#4f46e5]/30 focus:border-[#4f46e5] text-sm bg-[#f9fafb] focus:bg-white transition-all shadow-inner"
            >
        ...
        <div className="mt-8 pt-6 border-t border-[#e5e7eb] text-center">
          <p className="text-sm font-medium text-[#6b7280]">
            ...
            <Link href="/auth/login" className="text-[#4f46e5] hover:text-[#7c3aed] transition-colors font-bold hover:underline">
              Sign in securely
            </Link>
          </p>
        </div>
